<template>
    <v-app id="inspire">
      ...
      <v-app-bar>
        ...
        <v-app-bar-title>Personas</v-app-bar-title>
      </v-app-bar>
    ...
    >
      ...
      <template v-slot:top>
        <v-toolbar
          flat
        >
          <v-toolbar-title>Tabla de Personas</v-toolbar-title>
          <v-divider
            class="mx-4"
            inset
            vertical
          ...
          >
            ...
            <v-card>
              <v-card-title>
                <v-card-title class="text-h7">Añadir Persona</v-card-title>
              </v-card-title>
...
<v-dialog
          ...
          >
            ...
            <v-card>
              <v-card-title class="text-h7">Modificar Persona</v-card-title>
              <v-card-text>
                <v-container>
                  <v-row>
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: 'RFC', key: 'rfc', sortable: false, align: "center", width: "100px" },
      {
        title: 'Nombre',
        align: 'center',
        width: "100px",
  ...
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'Persona' : 'Editar Item'
    },
  },
  ...
  methods: {
    ...
    const jwtToken = localStorage.getItem("jwtToken");
      ...
      try {
      const response = await fetch('https://localhost:4001/personas', {
        headers: {
  ...
  try {
    ...
    if (!this.editedItem.nombre || !this.editedItem.rfc) {
      console.error('ID, Nombre y RFC son campos obligatorios.');
      return;
    }
    ...
    const response = await fetch(`https://localhost:4001/persona/${this.editedItem.id}`, {
      method: 'PUT',
      headers: {
    ...
    if (response.ok) {
      ...
      const index = this.datosPosts.findIndex(persona => persona.id === this.editedItem.id);
      if (index !== -1) {
        this.datosPosts[index].nombre = this.editedItem.nombre;
        this.datosPosts[index].rfc = this.editedItem.rfc;
      }
      ...
      console.error('Error al actualizar la persona:', response.statusText);
      this.mostrarMensaje('Error al actualizar la persona. Por favor, inténtalo de nuevo.');
    }
  } catch (error) {
    console.error('Error de red:', error.message);
  ...
  //fasdfsdf
    ...
    const personaId = this.editedItem.id;
    ...
    const response = await fetch(`https://localhost:4001/persona/${personaId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': "Bearer "+jwtToken,
    ...
    if (response.ok) {
      ...
      this.mostrarMensaje('Error al eliminar la persona. Por favor, inténtalo de nuevo.');
    }
  } catch (error) {
    console.error('Error de red:', error.message);
    this.mostrarMensaje('Error de red. Por favor, verifica tu conexión e inténtalo de nuevo.');
    ...
    // Validar la entrada antes de enviar la solicitud
    if (!this.editedItem.nombre || !this.editedItem.rfc) {
      console.error('Nombre y RFC son campos obligatorios.');
      return;
    }
    //console.log(" es = "+this.editedItem.id)
    ...
    const response = await fetch('https://localhost:4001/persona', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': "Bearer "+jwtToken,
    ...
    if (response.ok) {
      ...
      this.datosPosts.push(nuevaPersona);

      // Limpiar el formulario después de crear la nueva persona
      this.limpiarFormulario();
      ...
      this.mostrarMensaje('Error al crear la persona. Por favor, inténtalo de nuevo.');
    }
  } catch (error) {
    console.error('Error de red:', error.message);
    this.mostrarMensaje('Error de red. Por favor, verifica tu conexión e inténtalo de nuevo.');
