<template>
  <v-app id="inspire">
    ...
    <v-app-bar>
      ...
      <v-app-bar-title>Donadores</v-app-bar-title>
    </v-app-bar>
  ...
  >
    ...
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Tabla de Donadores</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
        ...
        >
          <template v-slot:activator="{ props }">
            <v-btn
              ...
              v-bind="props"
            >
              Añadir nuevo Donador
            </v-btn>
          </template>
          <v-card>
            <v-card-title>
              <v-card-title class="text-h7">Añadir Donador</v-card-title>
            </v-card-title>
...
<v-dialog
        ...
        >
          ...
          <v-card>
            <v-card-title class="text-h7">Modificar Donador</v-card-title>
            <v-card-text>
              <v-container>
                <v-row>
...
data: () => ({
  ...
  headers: [
    ...
    { title: 'ID Proyecto', key: 'idProyecto', sortable: false, align: "center", width: "100px" },
    { title: 'Cantidad Donada', key: 'cantDonada', sortable: false, align: "center", width: "100px" },
...
computed: {
  formTitle () {
    return this.editedIndex === -1 ? 'Donador' : 'Editar Item'
  },
},
...
methods: {
  async initialize () {
    ...
    try {
    const response = await fetch('https://localhost:4001/donadores', {
      headers: {
  ...
  const { id, idPersona, idProyecto, cantDonada } = realItem;

  // Asigna los valores al editedItem
  this.editedItem.id = id;
  this.editedItem.idPersona = idPersona;
  this.editedItem.idProyecto = idProyecto;
  this.editedItem.cantDonada = cantDonada;
...
try {
  ...
  if (!this.editedItem.idPersona || !this.editedItem.idProyecto || !this.editedItem.cantDonada) {
    console.error('Id Persona, Id Proyecto, y CantDonada son campos obligatorios.');
    return;
  }
  ...
  const response = await fetch(`https://localhost:4001/donador/${this.editedItem.id}`, {
    method: 'PUT',
    headers: {
    ...
    body: JSON.stringify({
      idProyecto: this.editedItem.idProyecto,
      idPersona: this.editedItem.idPersona,
      cantDonada: this.editedItem.cantDonada,
  ...
  if (response.ok) {
    ...
    const index = this.datosPosts.findIndex(donador => donador.id === this.editedItem.id);
    if (index !== -1) {
      this.datosPosts[index].idPersona = this.editedItem.idPersona;
      this.datosPosts[index].idProyecto = this.editedItem.idProyecto;
      this.datosPosts[index].cantDonada = this.editedItem.cantDonada;
    }
    ...
    this.mostrarMensaje('Donador actualizado exitosamente.');
  } else {
    console.error('Error al actualizar el donador:', response.statusText);
    this.mostrarMensaje('Error al actualizar el donador. Por favor, inténtalo de nuevo.');
  }
} catch (error) {
  console.error('Error de red:', error.message);
...
//fasdfsdf
  ...
  const response = await fetch(`https://localhost:4001/donador/${donadorId}`, {
    method: 'DELETE',
    headers: {
      'Authorization': "Bearer "+jwtToken,
  ...
  if (response.ok) {
    ...
    // Mostrar un mensaje de éxito al usuario
    this.mostrarMensaje('Donador eliminado exitosamente.');
  } else {
    console.error('Error al eliminar la donador:', response.statusText);
    this.mostrarMensaje('Error al eliminar el donador. Por favor, inténtalo de nuevo.');
  }
} catch (error) {
  console.error('Error de red:', error.message);
  this.mostrarMensaje('Error de red. Por favor, verifica tu conexión e inténtalo de nuevo.');
  ...
  // Validar la entrada antes de enviar la solicitud
  if (!this.editedItem.idPersona || !this.editedItem.idProyecto || !this.editedItem.cantDonada) {
    console.error('Id Persona, Id Proyecto y Cant Donada son campos obligatorios.');
    return;
  }
  //console.log(" es = "+this.editedItem.id)
  ...
  const response = await fetch('https://localhost:4001/donador', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': "Bearer "+jwtToken,
  ...
  if (response.ok) {
    const nuevoDonador = await response.json();
    console.log('Nueva donador creado:', nuevoDonador);

    // Actualizar la lista de personas después de crear una nueva
    this.datosPosts.push(nuevoDonador);

    // Limpiar el formulario después de crear la nueva donador
    this.limpiarFormulario();

    // Mostrar un mensaje de éxito al usuario
    this.mostrarMensaje('Donador creado exitosamente.');
  } else {
    console.error('Error al crear el donador:', response.statusText);
    this.mostrarMensaje('Error al crear el donador. Por favor, inténtalo de nuevo.');
  }
} catch (error) {
  console.error('Error de red:', error.message);
  this.mostrarMensaje('Error de red. Por favor, verifica tu conexión e inténtalo de nuevo.');
...
this.editedItem.idProyecto = '';
this.editedItem.cantDonada = '';
},
...
alert(mensaje);
  ...
  // Obtiene el idPersona del Autor a partir de su userID, recorre la lista hasta encontrar
